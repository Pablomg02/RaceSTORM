...
from ...utils.stochastic_funcs import gamma_rnd

class StochasticTire():
    ...
    def laptime(self, lap_stint : int, 
                a : float, scale : float,
                print_time : bool = False) -> float:
        """
        ...
        stochastic_time = gamma_rnd(a, time, scale)

        if print_time:
            print(f"Stint lap {lap_stint}: {stochastic_time:.2f} seconds")
        return stochastic_time, is_done